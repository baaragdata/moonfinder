...
class comm:
    ...
    def __init__(self,port='COM5'):
        ''' Init the COM port or UDP socket '''
       
        logging.basicConfig(
            format='%(asctime)s %(levelname)s:synscanComm %(message)s',
            level=LOGGING_LEVEL
            )
        logging.info(f"Com port target: {port}")
    ...
    def _send_raw_cmd(self,cmd,timeout_in_seconds=2):
        ...
        with self.lock:   
            ...
            if data:
                ...
                resp = data.split()
                resp = resp[1]
                logging.debug(f"response: {data[1]} port:{self._com_port}" )
            else:
                self.commOK=False
    ...
    def _send_cmd(self,cmd,axis,data=None,ndigits=6):
        ...
        if raw_response[0]==61:
            response=self._hex2int(raw_response[1:])
            #print(f'response from {__name__} = {type(response)}')
            return response
    ...
    def _hex2int(self,data):
        ...
        * 8 bits Data Sample: For HEX number 0x12, in the data segment of a command or
        ...
        if length==3:
            ...
            #print(f'3bytes response. Not converting to init. Returning as it as string')
            return strData
